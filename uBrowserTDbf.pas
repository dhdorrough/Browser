...
uses
  BDE, uBDEInfo, MW_Decl, StStrL, StStrZ, StBase, uModify_Table_Structure;
...
begin
  with Table1 do
    begin
      ...
      ModifyStructure1.Enabled  := Active;
...
procedure TForm_Browser.FindDialog1Find(Sender: TObject);
  ...
  var
    ...
    FldNr: integer;
{$IfDef UCZ}
    temp2: packed array[0..255] of char;
    LocalTable : BTable;   // Boyce-Moyer link table
    idx: cardinal;
{$EndIf}
{$IfDef UCL}
    temp2: string;
...
begin { TForm_Browser.FindDialog1Find }
  with FindDialog1 do
    begin
{$ifDef UCZ}
      BMMakeTableZ(pchar(MatchString), LocalTable);
{$EndIf}
{$IfDef UCL}
      BMMakeTableL(MatchString, LocalTable);
{$endif}
{$IfDef Pos}
            ...
            begin
              for i := 0 to Fields.Count-1 do
                with Fields[i] do
                  begin
{$ifDef UCZ}
                    StrPCopy(temp2, AsString);
                    FoundIt := BMSearchUCZ(temp2,  { dosen't seem to work }
                                    Length(AsString),
                                    LocalTable,
                                    pchar(MatchString),
                                    idx);
